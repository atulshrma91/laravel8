      ...
      <div class="col-md-12">
        <div class="card">
          ...
          <div class="card-body">
            ...
            <form method="POST" id="updateDealsCategoriesExtSettings" action="{{url('extensions/deals/categories/settings/update')}}">
              @csrf
            ...
            @endif
            <form method="POST" id="categoriesExtSettings" action="{{url('extensions/deals/categories/settings')}}">
              @csrf
              <div class="row">
                <div class="col-md-2">
                  <div class="form-group">
      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header card-header-info card-header-icon">
            ...
            <h4 class="card-title">Deal funnel Headlines</h4>
          </div>
          <div class="card-body">
            @if(!$deal_sources->isEmpty())
              @foreach($deal_sources as $deal_source)
              <div class="row">
                <div class="col-md-5">
                  <div class="form-group">
                    <input type="text" class="form-control" name="name" required="true" value="{{$deal_source->name}}">
                  </div>
                </div>
                <div class="col-md-7">
                  <div class="form-group">
                    <button type="button" class="btn btn-sm crm-btn-green update-deal-source" data-id="{{$deal_source->id}}">Update</button>
                    <button type="button" class="btn btn-sm btn-danger delete-deal-source" data-id="{{$deal_source->id}}">Delete</button>
                  </div>
                </div>
              </div>
              @endforeach
            @endif
            @if($deal_sources->count() < 3)
            <form method="POST" id="roleExtSettings" action="{{url('extensions/deals/settings')}}">
              @csrf
              <div class="row">
                <div class="col-md-5">
                  <div class="form-group">
                ...
                <div class="col-md-7">
                  <div class="form-group">
                    <button type="submit" class="btn btn-sm btn-info">Add Heading</button>
                    <a href="{{url('/extensions')}}" class="btn btn-sm btn-default">Cancel</a>
                  </div>
                </div>
              </div>
    ...
    jQuery(document).ready(function(){
      ...
      jQuery(document).on('click', '.delete-category', function(){
        ...
        swalWithMaterialButtons({
          ...
          preConfirm: (login) => {
            return fetch('/extensions/deals/categories/settings/'+id+'/delete', {
                method: 'GET',
                headers: {
      ...
      jQuery(document).on('click', '.update-deal-source', function(){
        jQuery(this).html('Update<i class="fa fa-spinner fa-spin"></i>');
        let id = jQuery(this).data('id')
        let name = jQuery(this).closest('.row').find('input[name="name"]').val()
        fetch('/extensions/deals/settings/'+id+'/update', {
            method: 'POST',
            headers: {
      ...
      jQuery(document).on('click', '.delete-deal-source', function(){
        let id = jQuery(this).data('id')
        swalWithMaterialButtons({
          title: `{{__('account.alert_heading')}}`,
          text: `{!!__('account.alert_text')!!}`,
          ...
          showLoaderOnConfirm: true,
          preConfirm: (login) => {
            return fetch('/extensions/deals/settings/'+id+'/delete', {
                method: 'GET',
                headers: {
                  'Content-Type': 'application/json;charset=utf-8'
                }
